...
- Expression range：中性战备、受击、签名技三种状态通过前端套印/VFX表达；首版不为每人生成三套完整立绘。
...
- Export：每位英雄保留独立透明 PNG 源图，并导出质量 88 的透明 WebP 供游戏加载。人物与卡框分离，卡框、图标、套印遮罩和状态变化使用前端 SVG/CSS。
...
- Card play：卡牌先向上 12 px 预备 55 ms，再沿直线进入舞台 105 ms；命中后缩回牌堆或碎成 4–7 个矩形。
- Hit：目标卡横移 3 px，朱红/青蓝双影分离 2 px 后在 90 ms 内归位。
- Signature：70 ms 视觉冻结感、18–24 个碎片、一次最大 3 px 舞台震动；总时长不超过 900 ms。
- Particles：矩形纸屑、三角裁角、短套印线；禁用圆形发光粒子。
