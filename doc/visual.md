...
- Action / upgrade-card illustration：六张基础行动牌与五张强化牌使用同一套 C「异能档案」生成式完整卡图，不能退回为 CSS 几何或单个 SVG 图标。突破为断裂切线、过载为闪电芯、架势为分层盾、反制为折返轨迹、探针为瞳孔定位环、校准为心跳与坐标轴；强化分别以裂解、守卫、序列、电池/修复液与锁定靶为象征物。图像必须在约 90 px 宽时仍能区分核心符号；前端只保留极小的类别角标和不透明文字带，不叠加裁切内框、折角或硬质外框。
- Rejected generation rule：出现手机外框、紫色霓虹、喧宾夺主的无关人像/动物、可读字母或透明棋盘预览的卡面一律拒绝，不因接口成功而接入；如画内包含行动者，只能退为核心象征物后的低对比背景，不能替代技能/强化的视觉语义。
