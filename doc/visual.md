...
- Frame language：A/B/C 的细薄边界、裁角与底部几何图标区均为完整插画内的一部分；前端容器只提供 8–12 px 圆角遮罩与低调投影，不再叠加统一硬框。
...
- Asset method：8 位首发人物与运行时新建的个人档案卡都使用 Aigram 平台 transit raster 生图接口，以公开头像/既有身份参考为依据，但直接生成一张完整场景插画，不生成透明人物层。运行时随机池仅包含已验收 A/B/C；D/E/F 只记录为未上线的扩展探索方向。身份参考只用于人物辨识，不使用截图中的帖子画面作为角色身份事实。生成记录保存在 `_artifacts/heroes/generation-log.json` 与个人存档中。
- Export：每位英雄导出质量 86–88 的不透明完整 WebP 卡面供游戏加载；历史 `cutouts/` 与 `states/` 只作为非运行时历史素材，不再被主界面导入。受击状态由前端专色裂痕、擦伤、暗角、套印错位和短暂位移表达。
- Interface raster：8 位行动员以完整 WebP 卡面作为玩家与敌对档案共同的画面层；敌对方通过暗红筛网、降饱和和档案标记区分。战斗桌与规则面板共享无人物的丝网印刷档案底板；战术牌和强化牌均以独立生成的完整 WebP 图像承担主视觉，CSS 只负责轻量状态、文字、圆角遮罩和响应式。
- Action / upgrade-card illustration：六张基础行动牌与五张强化牌使用同一套 C「异能档案」生成式完整卡图，不能退回为 CSS 几何或单个 SVG 图标。它们不是缩小的角色卡：画面只允许器物、几何、能量、矿物碎片、网点与印刷纹理，严禁人物、脸、身体、动物或角色剪影。突破为断裂切线、过载为闪电芯、架势为分层盾、反制为折返轨迹、探针为瞳孔定位环、校准为心跳与坐标轴；强化分别以裂解、守卫、序列、电池/修复液与锁定靶为象征物。图像必须在约 90 px 宽时仍能区分核心符号；前端只保留极小的类别角标和不透明文字带，不叠加裁切内框、折角或硬质外框。
- Rejected generation rule：出现手机外框、紫色霓虹、任何人物/动物/角色剪影、可读字母或透明棋盘预览的行动／强化卡图一律拒绝，不因接口成功而接入。人物行动员卡与纯符号行动／强化卡必须严格分工。
- Runtime card composition：运行时人物卡必须是完整、可独立成立的画面，而不是角色扣底 + 前端背景的拼贴。人物可站立、奔跑、交锋或与异常物互动；场景、道具、方向专属边界、仅由圆点/圆环/菱形/八芒星构成的能力带与象征物由模型在同一图中完成。必须保持脸部/身份锚点可辨，眼睛可见，但禁止强制所有角色采用胸像、三分之四视角或前端统一套上的固定底栏；严格禁止任何字母、数字、汉字、符文或伪文字。
- Runtime card material：完整卡面使用 8–12 px 圆角遮罩直接裁切插画，不添加厚重矩形外框。允许 1 px 半透明纸边高光与 `0 9px 20px` 的柔和炭黑投影，使其像可握持的实体收藏卡；文字/能力层只落在画内预留的低对比留白或图片之外的轻量信息区，不能覆盖人物脸部或重新把插画框成证件照。
...
- HUD：生命使用带身份标签、当前/最大大数字、断裂条和可选格挡读数的完整度仪表；当前生命数字在 390 px 宽时为 30 px，断裂条至少 11 px，敌人为朱红、玩家为青蓝但标签与数字始终同时出现。序列使用 3 个可点亮的偏移圆环；意图使用图标、动作名和具体数值三重表达。
- Default：炭纸面 + 骨白边。
...
- Living archive entry：随机三卡选择后进入一张不会自动消失的活体档案页。它以行动员肖像、三条编号协议和主按钮交代个人卡生成、对手封存、4/8/12 异变与持续至死亡的挑战；按钮确认后才进入首场。每轮第七份档案封存后使用不少于 2,400 ms 的黄铜章节页宣告下一轮威胁上升。
...
1. **A「漆金战谱 / Lacquer War Chronicle」**：黑漆、古铜与深朱红；角色在破损金箔、烟尘或兵器性异象中行动。整张图内置细薄的旧金裁角边界与底部抽象徽记带，构图庄重、有史诗对峙感；不使用历史文字或月亮。
2. **B「星骸圣典 / Astral Reliquary」**：深靛、骨白、旧金与星图般的轨迹；角色与异常天体碎片、几何器物或空旷遗迹发生关系。整张图内置纤细骨白/旧金的结构线、轨道几何与底部符号带；画面有秩序和留白，但不使用宗教标志或整齐头像照。
3. **C「异能档案 / Anomaly Dossier」**：炭黑、朱红、酸性青蓝的丝网印刷与套印偏移；角色处于正在失控或被标记的行动现场。整张图内置不规则裁切边、斜向专色区块与底部几何能力带；构图最锋利、细节最丰富，但不复刻前端的普通矩形面板。
4. **D「午夜行动片 / Midnight Operation」**：黑色电影式高反差侧光、潮湿街面、走廊或实验室；角色正在追逐、破门、抵抗或照见异常。强调镜头叙事和空间深度，色彩控制为墨黑、冷青、钠灯黄与一处危险朱红。
5. **E「矿物壁画 / Mineral Mural」**：厚重矿物颜料、石版画颗粒、大面积不规则色块与岩层/晶体般的环境。人物与异常物嵌在同一平面化构图中，色彩大胆但必须保留脸、眼睛和主要身份锚点。
6. **F「信号野外志 / Signal Field Guide」**：异常生态调查的野外场景，使用干刷、地图式地貌、风化纸张与机械化观测道具；角色可以与非人信号、生物或地景互动。它不是技术图纸，不放数字标尺、伪文字或卡牌边框。
...
- 禁止把圆形头像直接贴在卡面中央。
- 禁止把运行时新卡生成为透明扣底人像、纯背景胸像、统一证件照姿势，或由前端厚重框架掩盖大部分插画。
...
- Entry/start：390 × 844 首屏在 3 秒内看出这是“人物英雄卡牌战斗”；主英雄档案卡完整展示姓名、身份锚点、被动与签名技，单行索引完整展示 5 位候选人，不能出现两行头像格。已有个人卡时，个人卡固定占 1 位并沿用同一完整卡面承载方式，其余 4 位从首发池随机补齐；战斗阶段仍从完整首发人物池中调取敌人。
...
- Visual QA findings and decision：垂直切片阶段发现签名牌使用旧序列、320 宽单牌仅 99 px 两个 P1，均已修复。8 位首发扩展阶段完成中英文 390 × 844、英文 320 × 568、签名技、奖励和结算自动化检查；个人卡入池阶段再次覆盖 390 × 844、320 × 568、个人卡实战与 external-guest 状态。窄屏单牌达到 104 × 150，工具按钮 44 × 44，无横向溢出、资源加载错误或控制台错误。当前“8 位首发 + 1 位动态个人行动员”版本通过界面视觉门禁。
