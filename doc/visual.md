...
- Interface raster：8 位行动员以完整 WebP 卡面作为玩家与敌对档案共同的画面层；敌对方通过暗红筛网、降饱和和档案标记区分。战斗桌、战术牌、奖励与规则面板共享无人物的丝网印刷档案底板；位图负责完整构图与收藏感，CSS 只负责轻量状态、文字和响应式。
- Action-card illustration：六张基础行动牌必须使用同一套小型炭纸版画插图。突破为断裂切线、过载为闪电芯、架势为分层盾、反制为折返轨迹、探针为瞳孔定位环、校准为心跳与坐标轴；它们使用骨白、朱红、青蓝与薄荷四专色，居中占据卡面上半部，功能 SVG 图标只保留为类别角标。
