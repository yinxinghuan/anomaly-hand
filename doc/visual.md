...
- Asset method：8 位首发人物保留既有透明角色资产；运行时新建的个人档案卡使用 Aigram 平台 transit raster 生图接口，以公开头像为身份参考，但直接生成一张完整场景插画，不生成透明人物层。身份参考只用于人物辨识，不使用截图中的帖子画面作为角色身份事实。生成记录保存在个人存档中。
- Export：每位英雄保留独立透明 PNG 源图，并导出质量 86–88 的透明 WebP 供游戏加载；战损图位于 `img/heroes/states/`。人物与卡框分离，卡框、图标、套印遮罩和状态变化使用前端 SVG/CSS。
- Interface raster：8 位行动员以统一透明 WebP 作为玩家与敌对档案共同的人物层；敌对方通过暗红筛网、降饱和和档案标记区分。战斗桌、战术牌、奖励与规则面板共享无人物的丝网印刷档案底板；位图负责材质与收藏感，CSS 负责状态、文字和响应式。
...
- Runtime card composition：运行时人物卡必须是完整、可独立成立的画面，而不是角色扣底 + 前端背景的拼贴。人物可站立、奔跑、交锋或与异常物互动；场景、道具和象征物由模型在同一图中完成。必须保持脸部/身份锚点可辨，眼睛可见，但禁止强制所有角色采用胸像、三分之四视角、同一轮廓或固定底栏。
...
1. **A「漆金战谱 / Lacquer War Chronicle」**：黑漆、古铜与深朱红；角色在破损金箔、烟尘或兵器性异象中行动。构图庄重、有史诗对峙感，但不使用历史文字、月亮或固定金边卡框。
2. **B「星骸圣典 / Astral Reliquary」**：深靛、骨白、旧金与星图般的轨迹；角色与异常天体碎片、几何器物或空旷遗迹发生关系。画面有秩序和留白，但不使用宗教标志或整齐头像照。
3. **C「异能档案 / Anomaly Dossier」**：炭黑、朱红、酸性青蓝的丝网印刷与套印偏移；角色处于正在失控或被标记的行动现场。构图最锋利、细节最丰富，但不得把前端的固定档案框复画进图片。
